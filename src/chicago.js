const chicagoBackground = new Image();
chicagoBackground.src = '/src/chicago.png';

function checkChicagoLocations() {
    if (spriteX >= 110 && spriteX <= 140 &&
        spriteY >= 220 && spriteY <= 230) {
        /** REST STOP **/
        enterChicagoRestStop();
...
function enterChicagoRestStop() {
    console.log("Entering Rest Stop");
    document.removeEventListener('keydown', processPlayerInput)
    document.addEventListener('keydown', readChicagoRestStopInput);
    //canvas.addEventListener('click', handleCanvasClick);
    inRestStop = true;
}
...
function readChicagoRestStopInput(e) {
    switch (e.key) {
        case '1':
            inRestStop = false;
            player.money -= 200;
        ...
        case '2':
            ...
            document.removeEventListener('keydown', readChicagoRestStopInput);
            storeCarInGarage();
            loadCityChanges(City.NEW_YORK_CITY);
            break;
        case '8':
        case 'Escape':
            inRestStop = false;
